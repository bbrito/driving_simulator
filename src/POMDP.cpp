...
POMDP_Plan::POMDP_Plan(){
	...
	if (!n_.getParam(ros::this_node::getName()+"/init_ego_state", init_state))
	{
		ROS_ERROR("Parameter 'init_ego_state' not set");
		return;
	}
	else{
	...
	if (!n_.getParam(ros::this_node::getName()+"/init_obs_state", init_obs))
	{
		ROS_ERROR("Parameter 'init_obs_state' not set");
		return;
	}
	else{
		initi_obs_x = init_obs[0];
		initi_obs_y = init_obs[1];
		initi_obs_v = init_obs[2];
	}

	if (!n_.getParam(ros::this_node::getName()+"/n_points_spline", n_points_spline))
	{
		ROS_ERROR("Parameter 'n_points_spline' not set");
		return;
	}

	if (!n_.getParam(ros::this_node::getName()+"/acceleration", acceleration))
	{
		ROS_ERROR("Parameter 'acceleration' not set");
		return;
	}
...
State* POMDP_Plan::CreateStartState(std::string type) const{
	...
	if (!n_.getParam(ros::this_node::getName()+"/init_obs_state", state_A))
	{
		ROS_ERROR("State* POMDP_Plan::CreateStartState: Parameter 'init_obs_state' not set");
	}
...
Belief* POMDP_Plan::InitialBelief(const State* start, std::string type) const{
	...
	straight_state->state_A = state_A;
