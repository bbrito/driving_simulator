...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
	...
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [450 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.x0, 450);
    ...
    if( mxGetM(par) != 75000 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [75000 x 1]");
    }
#endif	 
    copyMArrayToC(mxGetPr(par), params.all_parameters, 75000);
	...
	copyCArrayToM( output.x01, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x01", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x02, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x02", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x03, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x03", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x04, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x04", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x05, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x05", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x06, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x06", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x07, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x07", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x08, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x08", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x09, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x09", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x10, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x10", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x11, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x11", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x12, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x12", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x13, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x13", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x14, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x14", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x15, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x15", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x16, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x16", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x17, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x17", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x18, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x18", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x19, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x19", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x20, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x20", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x21, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x21", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x22, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x22", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x23, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x23", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x24, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x24", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x25, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x25", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x26, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x26", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x27, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x27", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x28, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x28", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x29, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x29", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x30, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x30", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x31, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x31", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x32, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x32", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x33, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x33", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x34, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x34", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x35, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x35", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x36, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x36", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x37, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x37", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x38, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x38", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x39, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x39", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x40, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x40", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x41, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x41", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x42, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x42", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x43, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x43", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x44, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x44", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x45, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x45", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x46, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x46", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x47, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x47", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x48, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x48", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x49, mxGetPr(outvar), 9);
	mxSetField(plhs[0], 0, "x49", outvar);

	outvar = mxCreateDoubleMatrix(9, 1, mxREAL);
	copyCArrayToM( output.x50, mxGetPr(outvar), 9);
